    ...
    <body>
                        ...
                        <form action="${pageContext.request.contextPath}/BookAppointmentServlet" 
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="value-display">
                                        <c:choose>
                                            <c:when test="${not empty requestScope.slotTime and requestScope.slotTime != 'Chưa chọn' and requestScope.slotTime != 'Không tìm thấy khung giờ' and requestScope.slotTime != 'Khung giờ không xác định'}">
                                                ${fn:escapeXml(requestScope.slotTime)}
                                            </c:when>
                                            <c:otherwise>
                                                <span style="color: #dc3545; font-weight: bold;">
                                                    <i class="fas fa-exclamation-triangle"></i> Không tìm thấy khung giờ
                                                </span>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                    <input type="hidden" name="slotId" 
                                           value="${fn:escapeXml(not empty requestScope.slotId ? requestScope.slotId : (not empty param.slotId ? param.slotId : ''))}">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="value-display">
                                        <c:choose>
                                            <c:when test="${not empty requestScope.roomName and requestScope.roomName != 'Chưa có phòng' and requestScope.roomName != 'N/A' and requestScope.roomName != 'Phòng không xác định'}">
                                                ${fn:escapeXml(requestScope.roomName)}
                                            </c:when>
                                            <c:otherwise>
                                                <span style="color: #dc3545;">
                                                    <i class="fas fa-exclamation-triangle"></i> Chưa có phòng hoặc không xác định
                                                </span>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                    <input type="hidden" name="roomId" 
                                           value="${fn:escapeXml(not empty requestScope.roomId ? requestScope.roomId : (not empty param.roomId ? param.roomId : ''))}">
                                </div>
                            ...
                            <!-- Debug Section (Visible only in debug mode) -->
                            <c:if test="${initParam.debugMode == 'true'}">
                                <div class="debug-section">
                                    <h4><i class="fas fa-bug"></i> Debug Information</h4>
                                    <p>Slot ID: ${fn:escapeXml(requestScope.slotId)}</p>
                                    <p>Slot Time: ${fn:escapeXml(requestScope.slotTime)}</p>
                                    <p>Room ID: ${fn:escapeXml(requestScope.roomId)}</p>
                                    <p>Room Name: ${fn:escapeXml(requestScope.roomName)}</p>
                                    <p>Doctor ID: ${fn:escapeXml(requestScope.doctorId)}</p>
                                    <p>Patient ID: ${fn:escapeXml(requestScope.patientId)}</p>
                                    <p>Service ID: ${fn:escapeXml(requestScope.serviceIdTemp)}</p>
                                </div>
                            </c:if>
        ...
        <script>
            ...
            function validateAndSubmitForm(form) {
                ...
                const slotTime = "${fn:escapeXml(requestScope.slotTime)}";
                const roomName = "${fn:escapeXml(requestScope.roomName)}";
                ...
                const selectedServiceText = document.getElementById('serviceId').selectedOptions[0].text;
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                console.log("doctorId:", document.querySelector('input[name="doctorId"]').value);
                console.log("slotId:", document.querySelector('input[name="slotId"]').value);
                console.log("roomId:", document.querySelector('input[name="roomId"]').value);
                console.log("patientId:", document.querySelector('input[name="patientId"]').value);
                console.log("slotTime:", "${fn:escapeXml(requestScope.slotTime)}");
                console.log("roomName:", "${fn:escapeXml(requestScope.roomName)}");
                console.log("===========================");
            });
